...
class InputGuard:
    ...
    def protect(self, input_text: str, context: Optional[Dict] = None) -> GuardResult:
        ...
        try:
            ...
            risk_scores["security_scanning"] = self._compute_scan_risk_score(
                overall_scan
            )

            # Phase 2: Gradient-based Detection (if model available)
            if self.gradient_detector and self.config.get(
                ...
                detection_result = self.gradient_detector.detect(
                    input_text, compliance_response
                )

                if detection_result.is_threat:
                    violations.append(
                        f"Gradient anomaly detected: {detection_result.threat_type}"
                    )

                risk_scores["gradient_detection"] = detection_result.confidence_score

            # Phase 3: Refusal Feature Analysis (if model available)
            if self.refusal_detector and self.config.get(
            ...
            is_safe = (
                overall_risk < self.config.get("risk_threshold", 0.7)
                and len(violations) == 0
            )

            # Generate recommendations
            recommendations = self._generate_recommendations(
                violations, risk_scores, overall_risk
            )
            ...
            return GuardResult(
                ...
                sanitized_input=(
                    sanitized_input if sanitized_input != input_text else None
                ),
                ...
                metadata={
                    ...
                    "protection_layers": len(
                        [k for k, v in risk_scores.items() if v > 0]
                    ),
                },
            )
    ...
    def protect_multimodal(
        self, text: str = None, image_data: bytes = None, audio_data: bytes = None
    ) -> GuardResult:
        """Protect multi-modal inputs."""
        try:
            results = self.multimodal_scanner.scan_multimodal(
                text, image_data, audio_data
            )
    ...
    def _generate_recommendations(
        self, violations: List[str], risk_scores: Dict[str, float], overall_risk: float
    ) -> List[str]:
        ...
        if "prompt_injection" in str(violations):
            recommendations.append(
                "Potential prompt injection - validate input structure"
            )

        if risk_scores.get("gradient_detection", 0) > 0.7:
            recommendations.append(
                "Gradient anomaly detected - potential adversarial input"
            )
    ...
    def calibrate(self, safe_inputs: List[str], unsafe_inputs: List[str]):
        ...
        logger.info(
            f"Calibrated input guard with {len(safe_inputs)} safe and {len(unsafe_inputs)} unsafe examples"
        )
