...
class EvaluationRequest(BaseModel):
    ...
    benchmark: str = Field(
        ..., description="Benchmark name (jailbreakbench, harmbench)"
    )
...
class ShieldAPI:
    ...
    def _setup_routes(self):
        ...
        async def protect_input(
            request: ProtectionRequest, background_tasks: BackgroundTasks
        ):
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Protection failed: {str(e)}"
                )

        @self.app.post("/filter-output", response_model=OutputFilterResponse)
        async def filter_output(
            request: OutputFilterRequest, background_tasks: BackgroundTasks
        ):
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Filtering failed: {str(e)}"
                )
        ...
        async def evaluate_model(
            ...
            try:
                if request.benchmark.lower() == "jailbreakbench":
                    results = await self._run_jailbreak_evaluation(
                        request.model_name, request.sample_size
                    )
                elif request.benchmark.lower() == "harmbench":
                    results = await self._run_harm_evaluation(
                        request.model_name, request.sample_size
                    )
                else:
                    raise HTTPException(
        ...
        async def get_active_alerts():
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Alert retrieval failed: {str(e)}"
                )
    ...
    async def _run_jailbreak_evaluation(
        self, model_name: str, sample_size: int
    ) -> Dict[str, Any]:
        """Run JailbreakBench evaluation."""
        results = self.jailbreak_bench.evaluate(
            model_name=model_name, sample_size=sample_size, input_guard=self.input_guard
        )
        return results

    async def _run_harm_evaluation(
        self, model_name: str, sample_size: int
    ) -> Dict[str, Any]:
