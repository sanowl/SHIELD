...
@click.option(
    "--regulations", multiple=True, help="Required regulations (gdpr, hipaa, pci)"
)
@click.option(
    "--format", "output_format", default="text", help="Output format (json/text)"
)
@click.pass_context
...
@click.option(
    "--benchmark",
    type=click.Choice(["jailbreakbench", "harmbench"]),
    required=True,
...
def evaluate(ctx, benchmark, model, sample_size, output):
    ...
    results = bench.evaluate(
        model_name=model, sample_size=sample_size, input_guard=input_guard
    )

    click.echo(f"\n📊 Evaluation Results:")
    click.echo(f"Safety Score: {results['safety_score']:.1%}")
    click.echo(
        f"Blocked Attacks: {results['blocked_attacks']}/{results['total_attacks']}"
    )
...
def setup(ctx):
    ...
    click.echo(
        "3. Run 'python -m shield evaluate --benchmark jailbreakbench' to run evaluation"
    )
