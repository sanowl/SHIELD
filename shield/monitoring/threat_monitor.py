...
class ThreatMonitor:
    ...
    def log_detection(self, event_type: str, content: str, is_threat: bool, 
                     risk_score: float, **metadata):
        ...
        logger.info(f"Logged {event_type} event: threat={is_threat}, risk={risk_score:.3f}")
    ...
    def get_threat_trends(self, hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            ...
            "peak_risk_hours": [h for h, risks in hourly_risks.items() 
                              if risks and max(risks) > 0.8]
        }
    ...
    def _update_statistics(self, event: ThreatEvent):
        ...
        self.stats["avg_risk_score"] = ((current_avg * (total_events - 1)) + event.risk_score) / total_events
    ...
    def _check_alert_conditions(self, event: ThreatEvent):
        ...
        recent_threats = [e for e in list(self.events)[-20:] 
                         if e.is_threat and 
    ...
    def _generate_alert(self, level: AlertLevel, title: str, description: str, 
        ...
        alert = SecurityAlert(
            ...
            related_events=[f"{e.event_type}_{e.timestamp.isoformat()}" for e in related_events]
        )
    ...
    def _monitor_loop(self):
        ...
        while self._monitoring_active:
            try:
                ...
                if recent_stats["threat_rate"] > self.alert_thresholds["high_risk_rate"]:
                    self._generate_alert(
                        AlertLevel.MEDIUM,
    ...
    def _generate_recommendations(self, stats: Dict, alerts: List, trends: Dict) -> List[str]:
        """Generate actionable recommendations based on monitoring data."""
        recommendations = []
        
        if stats["threat_rate"] > 0.05:  # >5% threat rate
            recommendations.append("Consider tightening security thresholds due to high threat rate")
        
        if len(alerts) > 5:
            recommendations.append("Multiple active alerts - investigate potential coordinated attack")
        
        if trends["trend"] == "increasing":
            recommendations.append("Threat activity is increasing - consider enhanced monitoring")
        
        if stats["average_risk_score"] > 0.3:
            recommendations.append("Average risk score is elevated - review input validation")
