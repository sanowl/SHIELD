...
class GradientDetector(BaseDetector):
    ...
    def __init__(
        ...
        model,
        threshold: float = 0.7,
        safety_layer_names: Optional[List[str]] = None,
    ):
    ...
    def _compute_safety_gradients(
        self, input_text: str, response: str
    ) -> Dict[str, float]:
    ...
    def _identify_safety_parameters(self) -> List[str]:
        ...
        for name, param in self.model.named_parameters():
            ...
            if any(
                layer in name.lower() for layer in ["attention", "output", "classifier"]
            ):
    ...
    def calibrate_baseline(self, safe_inputs: List[str], safe_responses: List[str]):
        ...
        logger.info(
            f"Calibrated baseline gradients for {len(self.baseline_gradients)} parameters"
        )
...
class RefusalFeatureDetector(BaseDetector):
    ...
    def detect(self, input_text: str, response: str) -> DetectionResult:
        ...
        try:
            ...
            return DetectionResult(
                ...
                refusal_features=features.tolist()[
                    :50
                ],  # First 50 features for analysis
                metadata={
    ...
    def _detect_ablation_patterns(self, features: np.ndarray) -> float:
        ...
        # Combine indicators
        ablation_score = (
            0.6 * min(activation_variance * 10, 1.0) + 0.4 * near_zero_ratio
        )
    ...
    def train_refusal_features(self, harmful_inputs: List[str], safe_inputs: List[str]):
        ...
        logger.info(
            f"Trained refusal feature detector with {len(harmful_inputs)} harmful and {len(safe_inputs)} safe examples"
        )
